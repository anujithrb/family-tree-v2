/**
 * Shared tree operations service used by both API and Admin servers.
 *
 * The only behavioral difference is `editNode`:
 * - When `requestUserId` is provided (API), ownership checks restrict which fields
 *   a community admin can edit on a registered user's Person.
 * - When `requestUserId` is omitted (Admin), all fields are editable.
 */
import {
  Injectable,
  NotFoundException,
...
export class TreeOperationsService {
  ...
  /**
   * Edit a tree node's person data.
   *
   * @param requestUserId - If provided (API mode), applies ownership checks:
   *   community admins cannot edit name/birthYear/gender on a registered user's Person
   *   unless they ARE that user. If omitted (admin mode), all fields are editable.
   */
  async editNode(
    communityId: string,
    treeNodeId: string,
    ...
    // Admin-editable fields (always allowed)
    ...
    // Ownership-restricted fields
    const personOwnsAccount = node.person.user !== null;
    const requesterIsOwner = requestUserId != null && node.person.user?.id === requestUserId;
    const isAdminMode = requestUserId == null;

    if (!personOwnsAccount || requesterIsOwner || isAdminMode) {
